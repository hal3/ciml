...
% TODO: make sure this is covered in the previous chapter
...
an example as being reprsented by a \concept{feature vector}
...
\Figure{knn_projections}{A figure showing projections of data in
  two dimension in three ways -- see text.  Top: horizontal axis
  corresponds to the first feature (TODO) and the vertical axis
  corresponds to the second feature (TODO); Middle: horizonal is
  second feature and vertical is third; Bottom: horizonal is first and
  vertical is third.}
...
zero or one).  It is less clear what do do with \concept{categorical
...
\thinkaboutit{The computer scientist in you might be saying: actually
  we could map it to $\log_2 K$-many binary features!  Is this a good
  idea or not?}
...
\TODOFigure{knn:classifyit}{A figure showing an easy NN classification
  problem where the test point is a ? and should be positive.}
...
\Figure{knn_ski}{A figure of a ski and snowboard with width (mm)
  and height (cm).}

\Figure{knn_skidata}{Classification data for ski vs snowboard in
  2d}
...
(see Figure~\ref{fig:knn_ski}).  We are given two features about this
...
set; see Figure~\ref{fig:knn_skidata} where ski is the positive class.
Based on this data, you might guess that a KNN classifier would do
well.

\Figure{knn_skidatabad}{Classification data for ski vs snowboard in
...
Figure~\ref{fig:knn_skidatabad}.  Since the width values are now tiny,
...
\Figure{knn_db}{decision boundary for 1nn.}
...
sort will it classify as negative.  In Figure~\ref{fig:knn_db}, we have a
...
In Figure~\ref{fig:knn_db}, there is a solid line separating the positive
...
\Figure{knn_db}{decision boundary for knn with k=3.}
...
Figure~\ref{fig:knn_dbmany}, you can see the decision boundaries for KNN
models with $K \in \{1, 3, 5, 7\}$.  As you can see, the boundaries
become simpler and simpler as $K$ gets bigger.

\Figure{knn_dtski}{decision tree for ski vs. snowboard}
...
snowboards is shown in Figure~\ref{fig:knn_dtski}.

\Figure{knn_dtdb}{decision boundary for dt in previous figure}
...
decision tree in Figure~\ref{fig:knn_dtski} is shown in
Figure~\ref{fig:knn_dtdb}.  In the figure, space is first split in half
...
Figure~\ref{fig:knn_dtdb} is a good visualization of decision boundaries
...
simple.  
%We could also train a deeper decision tree, which yields the
%decision boundary in Figure~\ref{fig:knn:dtdeepdb}.  The depth of this
%tree is TODO, yielding a significantly more complex decision boundary.
...
\Figure{knn_clustering}{simple clustering data... clusters in UL,
  UR and BC.}

Consider the data shown in Figure~\ref{fig:knn_clustering}.  Since
...
Figure~\ref{fig:knn_clustering}.  The idea is to represent each
...
\Figure{knn_kmeans}{first few iterations of k-means running on
  previous data set}
...
Figure~\ref{fig:knn_kmeans}.  In this example, the clusters converge
very quickly.
...
Algorithm~\ref{alg:knn_kmeans} spells out the $K$-means clustering
...
%\TODOFigure{knn_kmeansbad}{first few iterations of k-means running on
...
Figure~\ref{fig:knn_kmeansbad}.  As you can see, the algorithm
...
\Figure{knn_grid_2d}{2d knn with an overlaid grid, cell with test
  point highlighted}
...
more like porcupines than like balls.\sidenote{This results was
...
\Figure{knn_cursetwo}{2d spheres in spheres}

Let's start in two dimensions as in Figure~\ref{fig:knn_cursetwo}.
...
\Figure{knn_cursethree}{3d spheres in spheres}

Now we can do the same experiment in three dimensions, as shown in
Figure~\ref{fig:knn_cursethree}.  Again, we can use the pythagorean
...
\Figure{knn_porcupine}{porcupine versus ball}

This is why we say that high dimensional spheres look like porcupines
and not balls (see Figure~\ref{fig:knn_porcupine}).  The moral of this
...
\TODOFigure{knn:uniform}{100 uniform random points in 1, 2 and 3
  dimensions}
...
\TODOFigure{knn:uniformhist}{histogram of distances in D=1,2,3,10,20,100}
...
\TODOFigure{knn:20ng}{histogram of distances in multiple D for 20ng}
...
\Figure{knn_badneighbors}{data set with 5nn, test point closest to
  two negatives, then to three far positives}

Regarding neighborliness, consider Figure~\ref{fig:knn_badneighbors}.
...
\Figure{knn_badneighbors2}{same as previous with $\ep$ ball}
...
report the majority class.  Figure~\ref{fig:knn_badneighbors2} shows an
